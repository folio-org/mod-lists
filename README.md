...
| S3_URL                                         | https://s3.amazonaws.com | Endpoint for the S3 bucket. For AWS, this value is ignored if S3_IS_AWS=true and S3_REGION is set (the region-specific endpoint will be used automatically). For MinIO or other S3-compatible storage, set this to your custom endpoint. |
...
| SYSTEM_USER_RETRY_MAX_ATTEMPTS                 | 15                       | Max number of retry attempts when waiting for the system user to be ready, for tenant migration operations                                                                                                                               |
